...
function MoviesDetails({ selectedId, setSelectedId, onAddWatched, movies }) {
  ...
  useEffect(
    function () {
      async function fetchMovie() {
        ...
        console.log('movie Test exist', data.imdbID)
        console.log('movie user rating', data)
        if (movies.find((movie) => movie.imdbID === data.imdbID)) {
          //add user rating of the movie finded to data
          data = { ...data, hideButton: true, userRating: movieUserRating }
          console.log('movie Test exist', data.imdbID)
        }
        setMovie(data)
      }
  ...
  const handleAddWatched = (movie) => {
    const newWatched = {
      ...
      year,
      poster,
      imdbIDRating: Number(imdbRating),
      ...
      <header>
        ...
        <div className="details-overview">
          {!movie.hideButton ? (
            ...
            <p>You rated this movie {movie.userRating}</p>
          )}
          <h2>{title}</h2>
          <p>{released} &bull</p>
        </div>
  ...
  return (
    <ul className="list list-movies">
      ...
      {movies.map((movie) => (
        <li key={movie.imdbID}>
          $
          <StarRating size={7} maxRating={movie.userRating} />
